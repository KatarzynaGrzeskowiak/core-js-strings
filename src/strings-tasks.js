...
function removeFirstOccurrences(str, value) {
  const included = str.includes(value);
  if (value.length > 0 && included) {
    const index = str.indexOf(value);
    const start = str.slice(0, index);
    const end = str.slice(index + value.length, str.length);
    const substr = start.concat(end);
    return substr;
  }
  return str;
}
...
function countVowels(str) {
  const count = Array.from(str).filter((letter) =>
    'aeiouy'.includes(letter.toLocaleLowerCase())
  ).length;
  return count;
}
...
function findLongestWord(sentence) {
  ...
  const sort = splitedSentence.sort((a, b) => b.length - a.length);
  return sort[0];
}
/**
...
function reverseWords(str) {
  ...
  const reverse = splited.map((item) => {
